  ...
  const {setLoading } = useAuth();
    ...
    const handleMakeDeliveryMan = user =>{
       ...
       const updatedUser = { ...user, role: 'deliveryMan' };
        fetch(`http://localhost:5000/users/deliveryman/${user._id}`,{
            method: "PATCH",
            headers: {
              "content-type": "application/json",
            },
    ...
    <tbody>
      {/* row 1 */}
      {
       ...
       { user.role === 'admin' ? 'Admin' : <button onClick={() =>handleMakeAdmin(user)} className="btn btn-lg bg-orange-600">
        ...
        <td>
       { user.role === 'deliveryMan' ? 'DeliveryMan' : <button onClick={() =>handleMakeDeliveryMan(user)} className="btn bg-blue-400">
